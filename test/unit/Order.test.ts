...
test("Deve tentar criar um pedido vazio com CPF invalido", function () {
  ...
  expect(() => new Order(cpf)).toThrow(new Error("CPF Invalido"));
});
...
test("Deve criar um pedido com 3 itens com um cupom de desconto expirado", function () {
  const cpf = "839.435.452-10";
  const order = new Order(cpf, new Date("2022-02-22"));
  order.addItem(new Item(1, "Música", "CD", 30), 3);
  order.addItem(new Item(2, "Vídeo", "DVD", 50), 1);
  order.addItem(new Item(3, "Vídeo", "VHS", 10), 2);
  order.addCoupon(new Coupon("VALE20", 20, new Date("2022-02-01")));
  const total = order.getTotal();
  expect(total).toBe(160);
});
...
test("Deve criar um pedido com 3 itens com calculo do frete com a estrategia default", function () {
  const cpf = "839.435.452-10";
  const order = new Order(cpf, new Date("2022-02-22"), new DefaultFreightCalculator());
  order.addItem(new Item(4, "Instrumentos Musicais", "Guitarra", 1000, 100, 30, 10, 3), 1);
  order.addItem(new Item(5, "Instrumentos Musicais", "Amplificado", 5000, 100, 50, 50, 20), 1);
  order.addItem(new Item(6, "Acessorios", "Cabo", 30, 10, 10, 10, 0.9), 3);
...
test("Deve criar um pedido com 3 itens com calculo do frete com a estrategia fixo", function () {
  const cpf = "839.435.452-10";
  const order = new Order(cpf, new Date("2022-02-22"), new FixedFreightCalculator());
  order.addItem(new Item(4, "Instrumentos Musicais", "Guitarra", 1000, 100, 30, 10, 3), 1);
  order.addItem(new Item(5, "Instrumentos Musicais", "Amplificado", 5000, 100, 50, 50, 20), 1);
  order.addItem(new Item(6, "Acessorios", "Cabo", 30, 10, 10, 10, 0.9), 3);
  const freight = order.getFreight();
